...
int
lsi_socket_connect(lua_State* L) {
    size_t endpoint_len;
    ...
    lsi_socket* sock = (lsi_socket*)lua_newuserdatauv(L, sizeof(lsi_socket), 0);
    if (sock == NULL) {
...
#ifdef _WIN32
    sock->hPipe =
        CreateFile(endpoint, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL);
    ...
    if (memcpy((void*)server_addr.sun_path, endpoint, endpoint_len + 1) == NULL) {
        return push_error(L, ERROR_FAILED_TO_CONNECT);
    }
...
int
lsi_socket_close(lua_State* L) {
    lsi_socket* sock = (lsi_socket*)luaL_checkudata(L, 1, LSI_SOCKET_METATABLE);
    if (sock == NULL) {
        return 0;
    }
    if (sock->closed) {
...
lsi_socket_read(lua_State* L) {
    ...
    if (timeout >= 0) {
        ...
        if (ReadFile(sock->hPipe, buffer, buffer_size, &bytes_read, &overlapped) == 0) {
            if (GetLastError() != ERROR_IO_PENDING) {
                CloseHandle(overlapped.hEvent);
                free((void*)buffer);
                return push_error(L, ERROR_READ_FAILED);
            }
            DWORD wait_res = WaitForSingleObject(overlapped.hEvent, timeout);
            ...
            if (GetOverlappedResult(sock->hPipe, &overlapped, &bytes_read, FALSE) == 0) {
                CloseHandle(overlapped.hEvent);
                free((void*)buffer);
                return push_error(L, ERROR_READ_FAILED);
            }
        ...
        if (ReadFile(sock->hPipe, buffer, buffer_size, &bytes_read, NULL) == 0) {
            free((void*)buffer);
            return push_error(L, ERROR_READ_FAILED);
        }
    }
...
int
lsi_socket_is_nonblocking(lua_State* L) {
    lsi_socket* sock = (lsi_socket*)luaL_checkudata(L, 1, LSI_SOCKET_METATABLE);
    if (sock == NULL) {
        return push_error(L, ERROR_SOCKET_IS_NIL);
    }
...
#ifdef _WIN32
    DWORD flags;
    if (GetNamedPipeHandleState(sock->hPipe, &flags, NULL, NULL, NULL, NULL, 0) == 0) {
...
int
lsi_socket_set_nonblocking(lua_State* L) {
    lsi_socket* sock = (lsi_socket*)luaL_checkudata(L, 1, LSI_SOCKET_METATABLE);
    if (sock == NULL) {
        return push_error(L, ERROR_SOCKET_IS_NIL);
    }
...
#ifndef _WIN32
void
push_address(lua_State* L, struct sockaddr* addr) {
    switch (addr->sa_family) {
        ...
        default: lua_pushnil(L); lua_pushnil(L);
    }
}
#endif

int
lsi_socket_get_peer_name(lua_State* L) {
    lsi_socket* sock = (lsi_socket*)luaL_checkudata(L, 1, LSI_SOCKET_METATABLE);
    if (sock == NULL) {
        return push_error(L, ERROR_SOCKET_IS_NIL);
    }
...
int
lsi_socket_equals(lua_State* L) {
    lsi_socket* sock1 = (lsi_socket*)luaL_checkudata(L, 1, LSI_SOCKET_METATABLE);
    lsi_socket* sock2 = (lsi_socket*)luaL_checkudata(L, 2, LSI_SOCKET_METATABLE);
...
int
lsi_socket_tostring(lua_State* L) {
    lsi_socket* sock = (lsi_socket*)luaL_checkudata(L, 1, LSI_SOCKET_METATABLE);
    if (sock == NULL) {
        return push_error(L, ERROR_SOCKET_IS_NIL);
    }
...
int
lsi_create_socket_meta(lua_State* L) {
